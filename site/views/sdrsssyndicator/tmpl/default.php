...
//print_r($rows);die();
	foreach ($rows as $row) {
		...
		//{jcomments on}
		$words = preg_replace('/{jcomments[\s]+on}/is','',$words);
        $words2 = preg_replace('/{jcomments[\s]+on}/is','',$words2);
		$words = preg_replace('/{jathumbnail[\s]+off}/is','',$words);
        $words2 = preg_replace('/{jathumbnail[\s]+off}/is','',$words2);
		//{phocagallery view=category|categoryid=65|imageid=2285}
		$words = preg_replace('/{[^{}]+}/is','',$words);
        $words2 = preg_replace('/{[^{}]+}/is','',$words2);
...
function delImagesFromHTML($html, $instances = -1) {
  $html = preg_replace('/<img[^>]*>/i','', $html, $instances);
...
function word_limiter($string, $limit = 100) {
	...
	//		$img=null;
	//		$i=0;
           // $imgsrc_regex = '#<img[^>]*src\s*=\s*(["\'])(.*?)\1#im';
	//		 $imgsrc_regex = '#<img[^>]*src\s*=\s*["\']([^"\']*?)["\'][^>]/>*#im';
        ...
        if (stripos($html, '<img') !== false) {
           ...
           $imgsrc_regex = '#<img[^>]*src\s*=\s*(["\'])(.*?)\1#im';
			...
			foreach ($matches[2] as $imagese) {
				...
				//print_r($purl);die();
				if (isset($purl['host'])) {
					$img[$i]='http://'.$purl['host'].$purl['path'];
				} else {
					$img[$i]='http://'.$root_url.$purl['path'];
				}
...
function allvideo($word) {
//print_r($word);
	//{avsplayer videoid=1 playerid=1}
	if (stripos($word, '{youtube}') !== false) {
		
		return true; //получаем массив id  видео.
	} else {
		return false;
	}
