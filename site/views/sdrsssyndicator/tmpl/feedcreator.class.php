...
class FeedItem extends HtmlDescribable {
	 ...
	 */
	var $author, $authorEmail, $authorURL,$image, $category, $section, $comments, $guid, $source, $creator, $contributor, $yandexgenre ;
...
class YandexrelatedItem extends HtmlDescribable{
	var $url,$text;
...
class HtmlDescribable {
	...
	function getDescription2() {
		...
		$descriptionField->syndicateHtml = $this->descriptionHtmlSyndicated;
		$descriptionField->truncSize = $this->descriptionTruncSize;
		return $descriptionField->output2();
	}
	...
	function _setFormat($format) {
		switch (strtoupper($format)) {
			...
			case "YANDEX":
				...
				break;
			case "RAMBLER":
			$this->_feed = new RAMBLERCreator();
				break;
			case "MAIL":
			$this->_feed = new MAILCreator();
				break;	
			//end cod
			default:
				$this->_feed = new RSSCreator091();
...
class FeedDate {
	 ...
	 */
	function FeedDate($dateString="", $zona="") {
...
/**
 * YANDEXCreator is a FeedCreator that implements RDF Site Summary (RSS) 2.0 vs Yandex,
...
/**
	...
	function createFeed() {
		...
		//$feed.= "<rss version=\"".$this->RSSVersion."\"  xmlns:atom=\"http://www.w3.org/2005/Atom\">\n";
		$feed.= "<rss version=\"".$this->RSSVersion."\"  xmlns=\"http://backend.userland.com/rss2\" xmlns:yandex=\"http://news.yandex.ru\">\n";
		$feed.= "    <channel>\n";
		$feed.= "        <title>".htmlspecialchars(htmlspecialchars_decode($this->title))."</title>\n";
		$this->descriptionTruncSize = 500;
		$feed.= "        <link>".$this->link."</link>\n";
		$feed.= "        <description>".$this->getDescription()."</description>\n";
		//$now = new FeedDate( );
		...
		if ($this->image!=null) {
			$feed.= "        <image>\n";
			$feed.= "            <url>".$this->image->url."</url>\n";
			$feed.= "            <title>".htmlspecialchars(htmlspecialchars_decode($this->image->title))."</title>\n";
			$feed.= "            <link>".htmlspecialchars(htmlspecialchars_decode($this->image->link))."</link>\n";
			$feed.= "        </image>\n";
		}
		...
		for ($i=0;$i<count($this->items);$i++) {
			$feed.= "        <item>\n";
			$feed.= "            <title>".htmlspecialchars(htmlspecialchars_decode(strip_tags($this->items[$i]->title)))."</title>\n";
			$feed.= "            <link>".htmlspecialchars(htmlspecialchars_decode($this->items[$i]->link))."</link>\n";
			// если будет нужен <pdalink> то следующюю строку раскоментируем
			//$feed.= "            <pdalink>".htmlspecialchars($this->items[$i]->link)."</pdalink>\n";


			$feed.= "            <description>".htmlspecialchars(htmlspecialchars_decode( strip_tags($this->items[$i]->description)))."</description>\n";

			if ($this->items[$i]->author!="") {
				$feed.= "            <author>".htmlspecialchars(htmlspecialchars_decode($this->items[$i]->author))."</author>\n";

			}

			if ($this->items[$i]->yandexgenre!="") {
			$feed.= "            <yandex:genre>".$this->items[$i]->yandexgenre."</yandex:genre>\n";
			...
			*/
			if ($this->items[$i]->category!="") {
				$feed.= "            <category>".htmlspecialchars(htmlspecialchars_decode($this->items[$i]->category))."</category>\n";
			}
			if ($this->items[$i]->comments!="") {
				$feed.= "            <comments>".htmlspecialchars(htmlspecialchars_decode($this->items[$i]->comments))."</comments>\n";
			}
			if ($this->items[$i]->date!="") {

			  $itemDate = new FeedDate($this->items[$i]->date, $this->items[$i]->zonatime );

				$feed.= "            <pubDate>".htmlspecialchars(htmlspecialchars_decode($itemDate->rfc822()))."</pubDate>\n";
			}
			...
			$feed.= $this->_createAdditionalElements($this->items[$i]->additionalElements, "        ");
			//print_r($this->items[$i]->additionalElements);
			//$this->items[$i]->enclosure = $this->items[$i]->additionalElements;
			if ($this->RSSVersion == "2.0" && $this->items[$i]->enclosure != NULL){
				foreach ($this->items[$i]->enclosure as $imagese)
					{
				                $feed.= "            <enclosure url=\"";
				                $feed.= htmlspecialchars(htmlspecialchars_decode($imagese->url));
			...
			if ($this->items[$i]->yandexrelated != NULL){
				...
				foreach ($this->items[$i]->yandexrelated as $ssilka)
					{
					$feed.= "            <link url=\"";
					$feed.= htmlspecialchars(htmlspecialchars_decode($ssilka->url));
					$feed.= "\">";
					$ssilka->text=preg_replace('#(?:& nbsp;)+#s', ' ', $ssilka->text);
					$feed.= htmlspecialchars(htmlspecialchars_decode($ssilka->text));
					$feed.= "</link>\n";
					}
				$feed.= "            </yandex:related>\n";
			}
			$feed.= "				<yandex:full-text>".htmlspecialchars(htmlspecialchars_decode( strip_tags(($this->items[$i]->descriptionfull),'<p>' )))."</yandex:full-text>\n";
...
class RAMBLERCreator extends FeedCreator {
var $deskrsort="";
	var $RSSVersion;

	function RAMBLERCreator() {
		$this->_setRSSVersion("2.0");
		$this->contentType = "application/rss+xml";
		$this->encoding = "utf-8";
	}
...
/**
	...
	function createFeed() {
		...
		$feed.= "<rss version=\"".$this->RSSVersion."\"  xmlns:rambler=\"http://news.rambler.ru\" >\n";
		$feed.= "    <channel>\n";
		$feed.= "        <title>".htmlspecialchars(htmlspecialchars_decode($this->title))."</title>\n";
		$this->descriptionTruncSize = 500;
		$feed.= "        <link>".htmlspecialchars(htmlspecialchars_decode($this->link))."</link>\n";
		$feed.= "        <description><![CDATA[".strip_tags(($this->description),'<p>')."]]></description>\n";
		//$now = new FeedDate( );
		//$feed.= "        <lastBuildDate>".htmlspecialchars($now->rfc822())."</lastBuildDate>\n";
		//$feed.= "        <generator>". $this->version()."</generator>\n";
		...
		for ($i=0;$i<count($this->items);$i++) {
			...
			$feed.= "            <title>".htmlspecialchars(htmlspecialchars_decode(strip_tags($this->items[$i]->title)))."</title>\n";
			$feed.= "            <link>".htmlspecialchars(htmlspecialchars_decode($this->items[$i]->link))."</link>\n";
			$feed.= "            <description><![CDATA[".strip_tags($this->items[$i]->description)."]]></description>\n";

			if ($this->items[$i]->author!="") {
				if ($this->items[$i]->authorEmail!="") {
					$feed.= "            <author> " . htmlspecialchars(htmlspecialchars_decode($this->items[$i]->authorEmail)) . " (".htmlspecialchars($this->items[$i]->author).")</author>\n";
				} else {
				      $feed.= "            <author> no_email@example.com (".htmlspecialchars($this->items[$i]->author).")</author>\n";
				}
			}
			...
			*/
			...
			if ($this->items[$i]->yandexrelated != NULL){
				...
				foreach ($this->items[$i]->yandexrelated as $ssilka)
					{
					$feed.= "            <rambler:related url=\"";
					$feed.= htmlspecialchars(htmlspecialchars_decode($ssilka->url));
					$feed.= "\" title=\"";
					$feed.= htmlspecialchars(htmlspecialchars_decode($ssilka->text));
					$feed.= "\" rel=\"news\" type=\"text/html\" pubdate=\"";
					$feed.= $itemDate->rfc822();
					$feed.= "\"/>\n";
					}

			}
			$feed.= "				<rambler:fulltext><![CDATA[".strip_tags(($this->items[$i]->descriptionfull), '<p>')."]]></rambler:fulltext>\n";
...
class MAILCreator extends FeedCreator {
var $deskrsort="";
	var $RSSVersion;
...
/**
	...
	function createFeed() {
		...
		//$feed.= "<rss version=\"".$this->RSSVersion."\"  xmlns:atom=\"http://www.w3.org/2005/Atom\">\n";
		$feed.= "<rss version=\"".$this->RSSVersion."\" xmlns:mailru=\"http://news.mail.ru/\" >\n";
		$feed.= "    <channel>\n";
		$feed.= "        <title>".htmlspecialchars(htmlspecialchars_decode($this->title))."</title>\n";
		$this->descriptionTruncSize = 500;
		$feed.= "        <link>".$this->link."</link>\n";
		$feed.= "        <description>".$this->getDescription()."</description>\n";
		$feed.= "		<language>ru</language>\n";
		$now = new FeedDate( );
		$feed.= "        <lastBuildDate>".htmlspecialchars($now->rfc822())."</lastBuildDate>\n";
		$feed.= "        <generator>SD RSS Syndicator for K2</generator>\n";
		...
		for ($i=0;$i<count($this->items);$i++) {
			...
			if ($this->items[$i]->guid!="") {
				...
				$feed.= "            <guid>".htmlspecialchars(htmlspecialchars_decode($this->items[$i]->link))."</guid>\n";
			}
			$feed.= "            <title><![CDATA[".strip_tags($this->items[$i]->title)."]]></title>\n";
			...
			$feed.= "			<mailru:full-text><![CDATA[". $this->items[$i]->descriptionfull."]]></mailru:full-text>\n";
			/*
			// on hold
			if ($this->items[$i]->source!="") {
					$feed.= "            <source>".htmlspecialchars($this->items[$i]->source)."</source>\n";
